...
class MultiGetId {
  ...
  private static final String TARGET_HOST =
      "alpha-analyticssearch-search-elastic.int.thomsonreuters.com";
  ...
  private static List<String> getListIdFromSourceIndex(Integer countOfId) {
    ...
    try {
      searchResponse =
          restHighLevelClientSource.search(
              createSearchRequestForSourceIndex(countOfId), COMMON_OPTIONS);
    } catch (IOException e) {
      e.printStackTrace();
    }
  ...
  private static MultiGetRequest createRequestForTargetIndex(List<String> listId) {
    ...
    for (String id : listId) {
      multiGetRequest.add(
          new MultiGetRequest.Item(TARGET_INDEX, id)
  ...
  private static List<String> checkDocsInTargetIndex(List<String> listId) {
    ...
    try {
      multiGetResponse =
          restHighLevelClientTarget.mget(createRequestForTargetIndex(listId), COMMON_OPTIONS);
    } catch (IOException e) {
      e.printStackTrace();
    }
  ...
  private static RestHighLevelClient makeConnection(
      String host, int portOne, int portTwo, String scheme) {

    return new RestHighLevelClient(
        RestClient.builder(
            new HttpHost(host, portOne, scheme), new HttpHost(host, portTwo, scheme)));
  }
